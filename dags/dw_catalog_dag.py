...
def _seed_s3_paths_from_sqls(connection, sql_files: list[Path]) -> None:
    ...
    for sql_file in sql_files:
        ...
        # We assume standard layout: a file defines schemas then views
        # For simplicity, we find all column definitions in the file
        cols = col_pattern.findall(content)
        if not cols:
            logger.info("No explicit schema found in %s, skipping rich seed.", sql_file.name)
            continue
            
        # Build a "Rich Seed" SQL part: CAST(NULL AS TYPE) AS name
        seed_select_parts = [f"CAST(NULL AS {ctype}) AS {name}" for name, ctype in cols]
        seed_sql = f"SELECT {', '.join(seed_select_parts)} WHERE 1=0"
        
        # Find S3 paths in this specific file
        ...
        for bucket, prefix in unique_prefixes:
            ...
            keys = s3_hook.list_keys(bucket_name=bucket, prefix=search_prefix, max_items=1)
            has_files = any(k.endswith(".parquet") for k in (keys or []))
            ...
            if not has_files:
                ...
                logger.info("Precision Seeding (Rich): Creating dummy for %s", seed_path)
